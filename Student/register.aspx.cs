        ...
        if (g.Length > 0 && c.Length > 0 && x.Length > 0 && n.Length > 0 && n.Length < 10)
        {
            if (LearnSite.Common.WordProcess.IsChina(n))
            {
                ...
                if (Sid > 1)
                {
                    ...
                    if (LearnSite.Ftp.FtpHelper.DatabaseExist())//如果存在ftp数据库，则创建ftp目录，添加到Webstudy,添加ftp账号
                    {
                        LearnSite.Ftp.Disk.CreateOneDir(mySyear, DDLclass.SelectedValue, Tsnum);
                        LearnSite.BLL.Webstudy ws = new LearnSite.BLL.Webstudy();
                        ws.AddOne(Tsnum, myPwd);
                        LearnSite.Ftp.Reg.RegsaveFtp(Tsnum, myPwd, mySyear, DDLclass.SelectedValue);
                    }
